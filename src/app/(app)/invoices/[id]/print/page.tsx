...
export default async function InvoicePrintPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const tax8Items = invoice.items.filter((it) => it.tax_rate === 8);
  const tax0Items = invoice.items.filter((it) => it.tax_rate === 0);
      ...
      <div className="invoice-wrap">
        ...
        <div className="meta-row">
          <div className="to-block">
            <div className="company">{invoice.partner?.name ?? "（取引先未設定）"} 御中</div>
        ...
        <div className="amount-box">
          <div className="label">ご請求金額（税込）</div>
          <div className="val">¥{fmt(invoice.total)} —</div>
        </div>

        <p style={{ fontSize: 13, marginBottom: 20 }}>下記のとおりご請求申し上げます。</p>
        ...
        <div className="clearfix">
          <table className="totals-table">
            <tbody>
              <tr><td>小計</td><td className="num">¥{fmt(invoice.subtotal)}</td></tr>
              {sum10 > 0 && (
                <tr>
                  <td>消費税（10%）</td>
                  <td className="num">¥{fmt(tax10)}</td>
                </tr>
              )}
              {sum8 > 0 && (
                <tr>
                  <td>消費税（8%）</td>
                  <td className="num">¥{fmt(tax8)}</td>
                </tr>
              )}
              {tax0Items.length > 0 && (
                <tr><td>非課税</td><td className="num">¥{fmt(tax0Items.reduce((s, i) => s + i.amount, 0))}</td></tr>
              )}
              <tr className="total-row">
                <td>合計（税込）</td>
                <td className="num">¥{fmt(invoice.total)}</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        {sum10 > 0 && (
          <div className="tax-note">
            ※ 10%対象：¥{fmt(sum10)}（消費税 ¥{fmt(tax10)}）
          </div>
        )}
        {sum8 > 0 && (
          <div className="tax-note">
            ※ 8%対象（軽減税率）：¥{fmt(sum8)}（消費税 ¥{fmt(tax8)}）
          </div>
        )}
        ...
        {invoice.notes && (
          <div className="notes-section">
            <strong>備考</strong>
            <p style={{ marginTop: 6, whiteSpace: "pre-wrap" }}>{invoice.notes}</p>
          </div>
        )}
      </div>
    </>
