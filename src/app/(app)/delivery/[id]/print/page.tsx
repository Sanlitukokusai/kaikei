...
import { getCompany, getDeliveryNote } from "@/lib/queries";
...
export default async function DeliveryPrintPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [dn, company] = await Promise.all([getDeliveryNote(id), getCompany()]);
  if (!dn) notFound();

  const tax10Items = dn.items.filter((it) => it.tax_rate === 10);
  const tax8Items = dn.items.filter((it) => it.tax_rate === 8);
  const tax0Items = dn.items.filter((it) => it.tax_rate === 0);
      ...
      <div className="invoice-wrap">
        ...
        <div className="meta-row">
          <div className="to-block">
            <div className="company">{dn.partner?.name ?? "（取引先未設定）"} 御中</div>
          ...
          <div className="from-block">
            ...
            <div>納品日：{fmtDate(dn.delivery_date)}</div>
            <div>納品書番号：{dn.delivery_no}</div>
          </div>
        </div>

        <div className="amount-box">
          <div className="label">納品金額（税込）</div>
          <div className="val">¥{fmt(dn.total)} —</div>
        </div>

        <p style={{ fontSize: 13, marginBottom: 20 }}>下記のとおり納品いたします。</p>
        ...
        <div className="clearfix">
          <table className="totals-table">
            <tbody>
              <tr><td>小計</td><td className="num">¥{fmt(dn.subtotal)}</td></tr>
              ...
              <tr className="total-row">
                <td>合計（税込）</td>
                <td className="num">¥{fmt(dn.total)}</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        {dn.notes && (
          <div className="notes-section">
            <strong>備考</strong>
            <p style={{ marginTop: 6, whiteSpace: "pre-wrap" }}>{dn.notes}</p>
          </div>
        )}
      </div>
    </>
