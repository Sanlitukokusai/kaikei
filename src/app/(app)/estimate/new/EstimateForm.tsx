...
import { createEstimate, type EstimateLineInput } from "@/app/actions/estimates";

type LineRow = EstimateLineInput & { _key: string };
...
export default function EstimateForm({
  ...
  const defaultValid = new Date(today.getTime() + 30 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);

  const [partnerId, setPartnerId] = useState<string>(initial?.partner_id ?? "");
  const [estimateNo, setEstimateNo] = useState<string>(initial?.estimate_no ?? `EST-${yyyymmdd}-001`);
  const [estimateDate, setEstimateDate] = useState<string>(initial?.estimate_date ?? today.toISOString().slice(0, 10));
  const [validUntil, setValidUntil] = useState<string>(initial?.valid_until ?? defaultValid);
    ...
    startTransition(async () => {
      ...
      try {
        if (mode === "edit" && initial) {
          const { updateEstimate } = await import("@/app/actions/estimates");
          await updateEstimate(initial.id, {
  ...
  return (
    <div className="yc-create">
          ...
          <div className="yc-form-block">
            <div className="yc-form-row">
              <div className="yc-form-col">
                ...
                <h3 className="yc-h3" style={{ marginTop: 18 }}>見積書情報</h3>
                <div className="yc-2col">
                  <Field label={<>見積日 <span className="yc-req">(必須)</span></>} span={12}>
                    <div className="yc-date">
                      <Input value={estimateDate} onChange={(e) => setEstimateDate(e.target.value)} />
                      <Icon name="Calendar" size={14} />
                    </div>
                  </Field>
                  <Field label="有効期限" span={12}>
                    <div className="yc-date">
                      <Input placeholder="yyyy-mm-dd" value={validUntil} onChange={(e) => setValidUntil(e.target.value)} />
                      <Icon name="Calendar" size={14} />
                    </div>
                  </Field>
                </div>
                <Field label={<>見積番号 <span className="yc-req">(必須)</span></>} span={12}>
        ...
        <div className="yc-right">
          <div className="yc-preview">
            <div className="yc-preview-head">
              <div style={{ fontSize: 11, color: "#64748b" }}>{estimateDate.replaceAll("-", "/")}</div>
              <div style={{ fontSize: 11, color: "#64748b" }}>見積番号：{estimateNo}</div>
            </div>
            <h2 className="yc-preview-title">見積書</h2>
            <div className="yc-preview-meta">
              <div>
                <div className="yc-preview-line" />
                <div style={{ marginTop: 12, fontSize: 12 }}>下記のとおり御見積申し上げます。</div>
                <div style={{ marginTop: 14, fontSize: 13 }}>
                  <div style={{ color: "#64748b", fontSize: 11 }}>御見積金額</div>
              ...
              <div style={{ textAlign: "right", fontSize: 11, lineHeight: 1.6 }}>
                ...
                {validUntil && <div>有効期限：{validUntil.replaceAll("-", "/")}</div>}
              </div>
            </div>
      ...
      <div className="yc-footer">
        ...
        <button type="button" className="yc-light-btn" onClick={() => router.push("/estimate")} disabled={isPending}>キャンセル</button>
        <button type="button" className="yc-primary-btn lg" onClick={onSubmit} disabled={isPending}>
          {isPending ? "保存中..." : mode === "edit" ? "変更を保存" : "見積書を保存"}
        </button>
      </div>
    </div>
  );
